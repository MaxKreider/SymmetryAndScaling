...
parfor aaa = 1:ntrials
    ...
    for qqq = 1:length(NN)
        ...
        for i = 1:size(X,1)
            ...
            p5 = local_row(X(i,:), S5(:,i));
            p6 = local_row(X(i,:), S6(:,i));

            % accumulate (P'*P)
            A = A + p1'*p1 + p2'*p2 + p3'*p3 + p4'*p4 + p5'*p5 + p6'*p6;
        end
...
function p = local_row(X, S)
    ...
    p = zeros(1,12);
    p(1)  = S(1);
    p(2)  = S(1)*X(1) - S(4)*X(4) - 2*S(6)*X(6) - S(7)*X(7);
    p(3)  = S(1)*X(2) - S(7)*X(6) - 2*S(8)*X(7) - S(5)*X(4);
    p(4)  = S(1)*X(3) - S(4)*X(4)^2 - S(7)*(2*X(4)*X(7) + X(6)*X(5)) - S(8)*(X(4)*X(8) + 2*X(7)*X(5)) - 3*S(6)*X(4)*X(6) - S(5)*X(4)*X(5);
    p(5)  = S(2);
    p(6)  = S(2)*X(1) - 2*S(6)*X(7) - S(4)*X(5) - S(7)*X(8);
    p(7)  = S(2)*X(2) - S(5)*X(5) - 2*S(8)*X(8) - S(7)*X(7);
    p(8)  = S(2)*X(3) - S(5)*X(5)^2 - S(6)*(2*X(4)*X(7) + X(6)*X(5)) - S(7)*(X(4)*X(8) + 2*X(7)*X(5)) - S(4)*X(4)*X(5) - 3*S(8)*X(5)*X(8);
    p(9)  = S(3);
    p(10) = S(4) + S(3)*X(1);
    p(11) = S(5) + S(3)*X(2);
    p(12) = S(3)*X(3) + S(4)*X(4) + S(6)*X(6) + S(7)*X(7) + S(5)*X(5) + S(8)*X(8);
end
